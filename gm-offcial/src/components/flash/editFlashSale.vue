<template>
  <div class="editFlashSaleWarp">
    <div class="title">编辑限时折扣</div>
    <div class="editFlashSaleCnt">
...
<script>
  ...
  export default{
    ...
    methods:{
      ...
      getActivityInfo(){
        let self = this;
        self.axios.post(Api.gtzApi + '/am/selectActivityInfoById', self.qs.stringify({
        	id:self.id
        }), {
        	headers: {
          ...
          if(res.data.code==1){
            self.activityTime = [self.formatDateTime(res.data.data.startTime),self.formatDateTime(res.data.data.endTime)];
            self.activityName = res.data.data.activityName;
            self.activityDesc = res.data.data.activityDesc;
            self.id = res.data.data.id;
            res.data.data.discountProducts.forEach(e=>{
              e.isJoin = true;
              e.sale = e.discount;
              e.reduce = (e.boneBeanPrice-e.activityPrice).toFixed(2);
              e.reduced = e.activityPrice.toFixed(2);
      ...
      commit(){
        ...
        if(this.activityName==''){
          ...
          if(!isCommit){
            ...
            self.axios.post(Api.gtzApi + '/am/updateDiscountProduct', self.qs.stringify({
              id:self.id,
