	...
	<div class="add_goods_warp">
		<ul class="add_goods_list">
			...
			<li class="clearfloat">
				...
				<div class="list_r">
					<el-cascader class="goodsClass"  placeholder="请选择商品分类" :options="goodsClassData" v-model="selectedGoodsClass" @change="handleChange">
					</el-cascader>
				</div>
			</li>
			<li class="clearfloat">
						    ...
						    style="width: 100%">
							  ...
							  <el-table-column
								...
								<template slot-scope="scope" >
									<span v-for="item in scope.row.guigeName">{{item.name}}</span>
								</template>
							  </el-table-column>
							 <el-table-column
								label="一级分类"
		...
		methods: {
			...
			addSpecification(){//添加规格
				if(this.specifications.length<2){
					let obj = {
							options: [{value: '尺寸',label:'尺寸'}, {value: '颜色',label:'颜色'}],
							value:'',input:'',guigeName:[],
			...
			addGuigeName(e){//添加规格名称
				let stockObj = {kucun:'',xiaoshoujia:'',chengbenjia:'',sku:'',name:''};
 				if(e.input!=""){
